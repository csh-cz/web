---
title: "Timometer – historické americké časové razítko po roce 1903"
slug: timometer
category: "projekty"
originalUrl: "https://hodinarium.eu/Timometer.htm"
lastModified: "Mon, 03 Nov 2025 17:07:00 GMT"
sourceCharset: "windows-1250"
scrapedAt: "2026-04-27T17:37:20.481Z"
tags:
  - pichacky
  - mechanicke
  - 1900s
tldr: 'Bostonský docházkový přístroj Johna C. Wilsona z přelomu století (patenty 1899 a 1903), předchůdce moderních píchaček a produkt firmy, jež se později stala součástí IBM.'
---
![Pohled na číselník](/img/Timometer/f/foto_0008.jpg)

V roce 2015 se na Aukru objevil Timometr a byl zakoupen soukromou osobou a umístěn v Hodináriu. Timometr se tak stane jedním ze dvou nejstarších "píchaček" v Hodináriu. AI k tomu dohledala následující informace:

### Historie a původ

Zařízení bylo vyvinuto v Bostonu (Massachusetts, USA) vynálezcem John C. Wilson a původně vyráběno firmou sídlící v této oblasti. Firma se později stala součástí International Time Recording Co., která se následně stala jedním ze základů dnešní IBM.

Na číselníku je vyraženo „Patented Nov. 21 ’99” a „Nov. 3 ’03”, což odpovídá dvěma americkým patentům:

- US 637 466 A („Timing apparatus”) udělenému 21. listopadu 1899

- US 743 801 A („Timing mechanism”) udělenému 3. listopadu 1903

Tato data potvrzují, že přístroj vznikl v rané éře mechanické automatizace záznamu pracovního času, krátce před masivním rozšířením zásobníků a hodinových registrátorů ve výrobních závodech.

### Účel a princip

Timometer byl přístroj sloužící k zaznamenávání přesného času pracovních příchodů a odchodů. V podniku měl každý zaměstnanec svoji kartu, kterou vložil do zařízení. Mechanismus uvnitř následně otiskl aktuální čas z hodinového strojku na kartu – podobně jako pozdější „píchačky”. Patent z roku 1899 popisuje: přesný převod hodinového strojku na razítkovací mechanismus, možnost automatického tisku vteřin, minut a hodin, a systém pro nastavení a synchronizaci více zařízení v jednom provozu.

### Klíčové prvky:

- Číselník s názvem **„Timometer – Boston, Mass. U.S.A.”**
- Vyražená značení „Patented Nov. 21 ’99”, „Nov. 3 ’03”, „Others Pending”
- Malý sekundový číselník nebo stupnice pro přesné měření času
- Mosazná skříň nebo čelní deska typické pro přelom století
- Mechanika realizující razítkovací úkon pomocí pákového nebo klikového mechanismu

### Inovace

Wilsonův vynález byl jedním z prvních, který kombinoval: klasické mechanické hodiny s automatickým záznamem času (time stamping) — proto se označení „Timometer” dá chápat jako „měřič času s tiskem”. Pozdější modely (okolo roku 1920) byly doplněny o elegantní mosazný číselník se sekundovou ručkou, jaký je i na zvažovaném exponátu. Patentové datum (Nov. 21, 1899 – Nov. 3, 1903) je vyraženo přímo na čelní desce originálních strojků.

### Historická hodnota

Tyto přístroje představují počátky průmyslové evidence pracovní doby a zároveň vývojový krok mezi k moderním docházkovým systémů. Každý dochovaný exemplář je dnes vzácným muzeálním předmětem, který dokumentuje přechod od řemeslné mechaniky k automatizaci – a zároveň vznik pozdějšího impéria IBM.

![Historická hodnota](/img/Timometer/f/foto_0001.jpg)

![Historická hodnota](/img/Timometer/f/foto_0010.jpg)

![Historická hodnota](/img/Timometer/f/foto_0002.jpg)[
](/img/Timometer/f/foto_0010.jpg)![Historická hodnota](/img/Timometer/f/foto_0007.jpg)

![Historická hodnota](/img/Timometer/f/foto_0008.jpg)

![Historická hodnota](/img/Timometer/f/foto_0009.jpg)

![Historická hodnota](/img/Timometer/f/foto_0003.jpg)

![Historická hodnota](/img/Timometer/f/foto_0004.jpg)

![Historická hodnota](/img/Timometer/f/foto_0005.jpg)

![Historická hodnota](/img/Timometer/f/foto_0006.jpg)

Ing. [Petr Král](/hodinari/petr-kral)
