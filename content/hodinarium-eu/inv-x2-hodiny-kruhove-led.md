---
title: '[K doplnění] Hodiny kruhové LED (inv. x1)'
slug: inv-x2-hodiny-kruhove-led
category: sbirka
podsekce: karta
karta:
  inventarniCislo: x2
  datace: '2025'
  signatura: PK
  ram: jiné
  kyvadlo: ''
  ciselnik: RGB LED kruh 24 x NeoPixel WS2812B
  pohonDetail: ESP8266
  rozmery: ''
  umisteni: Vitrína 4 (Sál elektro)
  majitel: Petr Král
  darceZapujcitel: Petr Král
  vztahKeSbirce: zápůjčka
  pridanoDoSbirky: 2025
  stav: funkční
  restaurovani: ''
  adaptaceProVystavu: ''
tags:
  - vitrina-4-vedlejsi
  - funkcni
author: Český spolek horologický
manualEdit: false
originalUrl: https://hodinarium-eu.pages.dev/sbirka/katalog
lastModified: null
sourceCharset: utf-8
scrapedAt: 2026-04-30T04:12:37.544Z
---

<aside class="editor-note editor-note-warn" data-source-file="content/hodinarium-eu/inv-x2-hodiny-kruhove-led.md" data-note-key="manual-edit-false" data-editor-only role="note" aria-label="Auto-import z legacy PHP">
  <header class="editor-note-head">
    <span class="editor-note-icon" aria-hidden="true">⚠</span>
    <span class="editor-note-title">Auto-import z legacy PHP webu</span>
    <span class="editor-note-meta" aria-hidden="true">viditelné jen přihlášenému editorovi</span>
  </header>
  <div class="editor-note-body">
    <p>Tato karta byla automaticky importována ze starých PHP stránek ČSH (frontmatter <code>manualEdit: false</code>). Obsah a metadata <strong>nebyly manuálně zkontrolovány</strong> — mohou obsahovat HTML/PHP artefakty, špatně zlomené citace, neúplné údaje. Po manuální revizi nastav <code>manualEdit: true</code>, aby tato hláška zmizela.</p>

    <button type="button" class="editor-note-resolve" data-action="resolve-editor-note">✓ Vyřízeno (smazat tuto poznámku z MDX)</button>
  </div></aside>
