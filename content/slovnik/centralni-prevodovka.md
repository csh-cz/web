---
title: centrální převodovka
slug: centralni-prevodovka
kategorie: mechanika
varianty:
  - centrální převodovka
  - centrální soukolí
  - central gear train (en)
  - centre wheel (en)
prekladyDe:
  - term: Minutenrad
    genus: n
  - term: Zentralrad
    genus: n
prekladyEn:
  - term: centre wheel
  - term: center wheel
prekladyFr:
  - term: roue de centre
    genus: f
  - term: roue centrale
    genus: f
definice: "**Středové (centrální) kolo** v hodinovém soukolí, jehož osa prochází středem ciferníku a na níž je nasazena minutová ručka. V klasickém hodinovém převodu obvykle **otáčí jednou za hodinu** (60 minut = 1 otáčka minutové ručky). Centrální převodovka je referenční bod, od kterého se odvozují všechny ostatní převody — k hodinové ručce (převod 1/12), k sekundové ručce (převod 60/1, pokud má hodiny střední sekundovku), k bicímu stroji, kalendáriu, atd."
pribuzne:
  - kralovska-hridel
  - krok
  - soukoli
  - krokove-kolo
isStub: true
references:
  - title: "Reid, T. *Treatise on Clock and Watch Making*"
    year: 1859
    type: kniha
    note: "Klasická anglická hodinářská učebnice — kompletní analýza standardního soukolí včetně středového kola (centre wheel)."
---

## Stručně

**Centrální převodovka** (něm. *Minutenrad* nebo *Zentralrad*, en. *centre wheel*, fr. *roue de centre*) je v běžné hodinové architektuře **třetí kolo od pohonu** (po hlavním pohonném válci a hnacím kole) a slouží jako **mechanický referenční bod**:

- Otáčí se **přesně jednou za hodinu** (60 minut)
- Na její osu je nasazena **minutová ručka**, která stejnou rychlostí obíhá ciferník
- Z ní se převodem **1 : 12** odvozuje hodinová ručka (osa hodinové ručky je obvykle dutá kolem osy centrálního kola)
- Z ní se převodem **1 : 60** odvozuje sekundová ručka (pokud je středová) — nebo se sekundová ručka pohání samostatným kolem („malá sekundovka")

## Funkce v různých typech hodin

| Typ hodin | Otáčky centrálního kola |
|---|---|
| Náramkové / kapesní | 1× za hodinu (standard) |
| Stolní pendlovky | 1× za hodinu |
| Věžní hodiny | 1× za hodinu (na centrální ose, přenos lanovým / hřídelovým převodem na ciferníky věže) |
| Astronomické hodiny | 1× za hodinu (referenční), z této osy se odvozuje pohon astronomického ciferníku, kalendária, fází měsíce |

## V Čechách — terminologická poznámka

Český termín *centrální převodovka* je **používán okrajově** — častější je popis **„centrální (středové) kolo"** nebo **„kolo s minutovou ručkou"**. Anglický pojem *centre wheel* je v hodinářské literatuře (Reid 1859, Britten 1922) standardní, v české odborné literatuře (Sladkovský 1947, Hajn 1953, Bureš 1965) se používá popisně.

V kontextu **věžních hodin** a **orlojů** se mluví o **„hlavním kole minutové převodovky"** nebo prostě o **„minutovém kole"** (např. v restaurátorské dokumentaci Petra Skály).

## Související

- [krok](/slovnik/krok), [krokové kolo](/slovnik/krokove-kolo) — kroky před centrálním kolem v převodovém řetězci
- [setrvačka](/slovnik/setrvacka), [vlásek](/slovnik/vlasek) — oscilátor na konci převodu (před krokem)
- [ručka orloje](/slovnik/ruka-orloje) — pevně spojená s centrálním kolem
