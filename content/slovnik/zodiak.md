---
title: zodiak
slug: zodiak
kategorie: astronomicke
varianty:
  - zodiak
  - zodyak
  - zvěrokruh
  - zodiakův okolek
prekladyLa:
  - term: zodiacus
    genus: m
    zdroj: 'Balbín/Šlajer 1681'
definice: "Zvěrokruh — pás 12 zvířetníkových znamení v ekliptice, po kterém zdánlivě postupuje slunce, měsíc a planety. Na Staroměstském orloji je zobrazen jako otáčející se plech s namalovanými symboly."
pribuzne:
  - sphera
  - grad-zodiakalni
  - strych-zodiakalni
isStub: true
references:
  - bibKey: taborskyzklokotskehoryZpravaStaromestskemOrloji1570
    citace: "Postupují slunce a měsíc po zodiaku, tak jako na nebi; přišroubovaný jiný okolek široký plechový pozlacený."
    note: "Rukopis AHMP, fond 3 (Sbírka úředních knih a rukopisů), sign. 1867."
  - bibKey: teigeJanaTaborskehoKlokotske1901
    note: "Tištěná edice s úvodem (J. Teige, Praha 1901)."
---

## Výklad

Z řec. *zōdiakòs (kýklos)* = zvířecí (kruh). Latinizovaný tvar v češtině 16. století psán *zodiak* nebo *zodyak*. V moderní češtině: **zvěrokruh**. Etymologicky souvisí se *zóon* (= zvíře) — protože většina znamení nese jména zvířat.

## Příbuzné termíny

[sphera](/slovnik/sphera), [grad-zodiakalni](/slovnik/grad-zodiakalni), [strych-zodiakalni](/slovnik/strych-zodiakalni)

## Obrázky

*(zatím chybí — k doplnění z fotodokumentace Staroměstského orloje nebo z edice Teige 1901)*
