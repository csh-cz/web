---
title: Brocotův krok
slug: brocotuv-krok
kategorie: mechanika
prekladyDe:
  - term: Brocots Stiftengang
    genus: m
    zdroj: "DE — Dietzschold 1905"
  - term: Brocot-Hemmung
    genus: f
    zdroj: "DE eponym"
  - term: Stiftenanker
    genus: m
    zdroj: 'DE varianta — „kotva s kolíčky" (rys Brocotovy konstrukce)'
prekladyEn:
  - term: Brocot escapement
    zdroj: "EN eponym"
  - term: pin pallet escapement
    zdroj: "EN obecný — kotva s kolíčky (zahrnuje i Roskopfa)"
prekladyFr:
  - term: échappement Brocot
    genus: m
    zdroj: "FR eponym primární"
  - term: échappement à goupilles
    genus: m
    zdroj: "FR — kotva s kolíčky"
varianty:
  - term: Brocotův krok
    status: preferred
    note: "Moderní cs eponym."
  - term: krok Brocotův
    status: admitted
    note: "Inverze."
  - term: krok kotvový kolíčkový s kolíčkovou kotvou
    status: admitted
    note: "Strukturovaný popisný — varianta B kolíčkového kroku."
    doloženo: "Slovník: kolíčkový krok, varianta B"
pribuzne:
  - krok
  - kolickovy-krok
  - amantuv-krok
  - kotva
definice: 'Klidový kotvový krok s kolíčky tvořícími palety kotvy. Krokové kolo má normální ostré zuby, ale kotva je nahrazena dvěma poloválcovými kolíčky. Vynalezl Achille Brocot kolem 1850. Často integrovaný do osy kyvadla, viditelný zepředu ciferníku — typická vlastnost pařížských pendulí 19. století („Brocotova kotva" jako dekorativní prvek).'
crossRefs:
  kroky:
    - brocotuv-krok
references:
  - bibKey: kralovstviHodin
    title: "Království hodin — Slovníček"
    url: "https://kralovstvihodin.cz/slovnicek/"
    note: '„krok kotvový kolíčkový — varianta 2) s kolíčky nahrazujícími palety na kotvě — krok Brocotův, krok Roskopfův.'
    key: kh-brocotuv
  - bibKey: hajn1953jemnaMechanika
    title: "Hajn M., *Základy jemné mechaniky a hodinářství*. Praha 1953."
    note: "Obrázek 144 — Brocotův krok."
    key: hajn-1953
    zoteroKey: 9MK4QIHS
  - bibKey: dietzschold1905
    title: "Dietzschold C., 1905."
    note: '„Brocots Stiftengang" — DE odborný termín.'
    key: dietzschold-1905
---

## Výklad

**Klidový kotvový krok s kolíčkovou kotvou** — vynalezl francouzský hodinář **Achille Brocot** (1817–1878) kolem **1850**. Konstrukčně:

- **Krokové kolo** má **normální ostré zuby** (ne kolíčky jako Amantův typ!)
- **Kotva** je tvořena **dvěma poloválcovými kolíčky** (rubínovými nebo z kalené oceli) — jeden vstupní, druhý výstupní

Tím Brocot **kombinuje** dvě dříve oddělené tradice: ozubené krokové kolo (Graham 1715) a kolíčkové palety (varianta Amantova kolíčkového kroku).

**Charakteristický rys pařížských pendulí 19. století** — Brocotova kotva je často **integrovaná do osy kyvadla** a **viditelná zepředu ciferníku**. Vzniká tak dekorativní prvek: kyv kyvadla se promítá do oscilace kotvy mezi zuby krokového kola, což je pohled atraktivní pro „pendulové umění" 2. císařství a Bel Epoque. **„Brocotova kotva"** je dnes synonymum pro tyto **viditelné krokové ústrojí** v měšťanských pendulových hodinách.

Brocot vyvinul rovněž **Brocotovu regulaci** — mikroregulační šroub pro nastavení délky kyvadla; ten je rovněž často viditelný na ciferníku (typicky nad číslicí 12, dvojice šipek označená „R" a „A" — *retard*/*avance*).

**Pro velké hodiny** se Brocotův krok běžně **nepoužíval**, je vyhrazen **stolním a nástěnným hodinám** 19. století. Pro věžní hodiny převládá Grahamův krok nebo specializované gravitační kroky.

## Konstrukce a princip činnosti

Detailní popis → **[Brocotův krok v sekci Kroky](/kroky/brocotuv-krok)**.

## Související

- [Amantův krok](/kroky/amantuv-krok) — varianta s kolíčky na krokovém kole (1741)
- [kolíčkový krok](/slovnik/kolickovy-krok) — generická kategorie obou typů
- [krok](/slovnik/krok), [kotva](/slovnik/kotva) — slovník
