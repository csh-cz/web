---
title: volovo (olovo)
slug: volovo
kategorie: materialy
varianty:
  - volovo
  - olovo
  - volověná váha
definice: "Staročeský pravopis slova *olovo* — s protetickým *v-*. U Táborského *volovo* označuje specificky **olověnou vyvažovací zátěž**, vlitou do kola slunce nebo měsíce pro vyvážení excentrického zatížení (např. olovo + indikátor na opačné straně osy)."
pribuzne:
  - vocel
  - okolek
isStub: true
references:
  - bibKey: taborskyzklokotskehoryZpravaStaromestskemOrloji1570
    citace: "Dotýče volova vpraveného do kola slunce a měsíce; volověná přivázána v kraji okolku."
    note: "Rukopis AHMP, fond 3 (Sbírka úředních knih a rukopisů), sign. 1867."
  - bibKey: teigeJanaTaborskehoKlokotske1901
    note: "Tištěná edice s úvodem (J. Teige, Praha 1901)."
---

## Výklad

Princip — protiváha pro vyvážení excentrického indikátoru — je dodnes standardní u všech indikátorových mechanismů (např. ve velkých kalendářních ciferníkách). V moderní češtině: **olověná protiváha**, **vyvažovací závaží**. Změna *v-/u- → o-* (paralelně s *vocel → ocel*, *vobruč → obruč*) je rys 17.–18. století.

## Příbuzné termíny

[vocel](/slovnik/vocel), [okolek](/slovnik/okolek)

## Obrázky

*(zatím chybí — k doplnění z fotodokumentace Staroměstského orloje nebo z edice Teige 1901)*
