---
title: početník
slug: pocetnik
kategorie: bici
prekladyDe:
  - term: Rechen
    zdroj: "Špatný 1882: Rechen → početník"
prekladyEn:
  - term: rack
prekladyFr:
  - term: râteau
varianty:
  - početník (moderní)
  - "počítadlo** (méně častá varianta)"
definice: "Ozubený segment v bicím stroji typu „s početníkem a stupnicí\" (Sladkovský 1947 typ II), který svým **vypadnutím** předem započte počet úderů, jež má bicí stroj odbít."
pribuzne:
  - bicí-stroj
  - srdcovka
  - kolo-závěrkové
  - stupnice
  - posuvka
---

## Výklad

Početník je **klíčová součást novějšího typu bicího stroje** (Sladkovský 1947 typ II), který nahradil starší typ se závěrkovým kolem. Hlavní výhoda: bití lze **opakovat** bez narušení sekvence (umožnilo to vznik **opakovacích hodinek**, repetic).

Funkce: V okamžiku spuštění bití početník **vypadne** (Špatný „der Rechen fällt aus") a opře se jedním ramenem o **stupnici** (12-stupňový kotouč spojený s hodinovou ručkou). Hloubka, do níž početník vypadne, závisí na výšce příslušného schůdku stupnice — a tím **přímo určuje počet úderů**.

Při každém úderu kladívka **západka** (kterou pohybuje srdcovka nebo posůvka) zvedne početník o jeden zub. Když poslední zub mine, **zubový kruh skončí** → bicí stroj se zastaví.

Špatný 1882 dokumentuje příbuzné odvozeniny:
- **Rechenarm** = rameno početníku
- **Rechenauflage** = položka početníku
- **Rechenfeder** = zpruha početníku
- **Rechenmeissel** = značník početníku
- **Rechenzahn** = zub početníku

Sladkovský přidává: **početník čtvrťový** má jen 6–7 zubů (bije nejvýš 4×3 = 12 úderů, ne 4×4); **početník hodinový** má 12 zubů.

## Příbuzné termíny

[bicí stroj](#bicí-stroj), [stupnice], [srdcovka](#srdcovka), [posůvka], [západka početníková], [kolo závěrkové](#kolo-závěrkové)

## Reference

> „Rechen m. početník; der Rechen fällt aus početník vypadá; der Rechen fällt nicht ganz početník úplně nevypadá. Rechenarm m. rameno početníku. Rechenfeder f. zpruha početníku. Rechenzahn m. zub početníku."
> — ŠPATNÝ, František. *Deutsch-böhmisches Wörterbuch.* 1882, heslo Rech–. [Zotero `D2A35XU2`]

> „Bicí stroj s početníkem a stupnicí. Jest to pozdější vynález a liší se od prvního způsobu hlavně tou výhodou, že uplynulou hodinu možno libovolně nechat opakovati, aniž by tím nastala změna v pořadí dalšího bití."
> — SLADKOVSKÝ, Josef. *Učebnice odborné nauky hodinářské.* 1947, kapitola Zařízení strojů bicích. [Zotero `VIBRCUZT`]

> „Početník čtvrťový má jen 6–7 zubů, posůvka jedno křídlo a zub. Stupnice o 4 stupních sedí na kole střídném a nese 4 kolíčky výpustné, které zvedají spoušť výmrštnou."
> — SLADKOVSKÝ, Josef. *Učebnice odborné nauky hodinářské.* 1947. [Zotero `VIBRCUZT`]

## Obrázky

*(zatím chybí — návrh: schéma početníku se stupnicí, foto z věžních hodin)*
