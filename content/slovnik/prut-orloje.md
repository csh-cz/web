---
title: prut (rameno kola)
slug: prut-orloje
kategorie: mechanika
varianty:
  - term: prut
    status: archaic
    note: 'Staročes. termín (Táborský 1570) — rameno / paprsek rotujícího kola, zejména trojpaprskového kola sphery (astroláb).'
    doloženo: 'Táborský 1570'
  - term: rameno
    status: preferred
    note: 'Moderní cs hodinářský termín pro paprsek kola (Bureš, Martínek-Řehoř).'
  - term: paprsek
    status: admitted
    note: 'Synonymum k „rameno"; běžné v restaurátorské literatuře.'
prekladyDe:
  - term: Speiche
    genus: f
    zdroj: 'DE moderní — rameno / paprsek kola'
  - term: Radarm
    genus: m
    zdroj: 'DE — paprsek kola'
  - term: Arm
    genus: m
prekladyEn:
  - term: spoke
    zdroj: 'EN standardní — paprsek kola'
  - term: wheel arm
    zdroj: 'EN popisný'
  - term: arm
prekladyFr:
  - term: rayon
    genus: m
    zdroj: 'FR — paprsek'
  - term: bras de roue
    genus: m
    zdroj: 'FR popisný'
  - term: branche
    genus: f
prekladyLa:
  - term: pertica
    genus: f
    zdroj: 'Balbín/Šlajer 1681'
definice: 'Rameno / paprsek kola. U Táborského *kolo tréprutové* = kolo se třemi rameny (paprsky), na kterém sedí indikátory orloje.'
pribuzne:
  - oblouk-orloje
  - sphera
  - krokove-kolo
isStub: true
references:
  - bibKey: taborskyzklokotskehoryZpravaStaromestskemOrloji1570
    citace: "Toliko tři pruty vztahující se k krajuom sphery."
    note: "Rukopis AHMP, fond 3 (Sbírka úředních knih a rukopisů), sign. 1867."
  - bibKey: teigeJanaTaborskehoKlokotske1901
    note: "Tištěná edice s úvodem (J. Teige, Praha 1901)."
  - bibKey: spatny1882nemeckoCeskySlovnik
    title: 'Špatný F., *Německo-český slovník pro hodináře a pouzdráře hodinářské*. 1882.'
    note: 'Hesla Radarm, Speiche — DE odpovídající moderní cs „rameno kola" / „paprsek".'
    key: spatny-1882
---

## Výklad

V moderní češtině: **rameno** nebo **paprsek kola** (v případě rotujícího kola), **prut** zachováno jen pro tenké rovné kovové součásti (např. prut kotvy). Trojprutá struktura je staročeský konstrukční prvek, dnes nahrazený plnotvarým kolem nebo speciálním pavoukovitým rámem.

## Příbuzné termíny

[oblouk-orloje](/slovnik/oblouk-orloje), [sphera](/slovnik/sphera)

## Obrázky

*(zatím chybí — k doplnění z fotodokumentace Staroměstského orloje nebo z edice Teige 1901)*
