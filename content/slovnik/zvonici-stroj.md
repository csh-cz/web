---
title: zvonicí stroj
slug: zvonici-stroj
conceptId: HORO-GEARTRAIN-004
kategorie: bici
varianty:
  - term: zvonicí stroj
    status: preferred
    note: 'Primární cs odborný termín — analogie k [bicímu stroji](/slovnik/bici-stroj). Doložen Skálou 2013 v kontextu vypouštění Pražského orloje („kolíčkem na hlavním kole zvonicího stroje").'
    doloženo: 'Skála 2013'
  - term: soukolí zvonicí
    status: admitted
    note: 'Druhá varianta — analogie k „soukolí jicímu" (Sladkovský 1947).'
  - term: zvonicí soukolí
    status: admitted
  - term: soukolí zvonkohry
    status: admitted
    note: 'Specifické pro hodinky/věžní hodiny s víctónovou melodickou zvonkohrou.'
prekladyDe:
  - term: Spielwerk
    genus: n
    zdroj: 'Berner, *Illustriertes Fachlexikon der Uhrmacherei*; Špatný 1882'
  - term: Glockenspielwerk
    genus: n
    note: 'Pro skutečnou zvonkohru s víctónovou melodií.'
prekladyEn:
  - term: chiming train
    zdroj: 'Berner, *Illustrated Professional Dictionary of Horology*'
  - term: chime train
prekladyFr:
  - term: rouage de carillon
    genus: m
    zdroj: 'Berner, *Dictionnaire professionnel illustré de l''horlogerie*'
  - term: carillon
    genus: m
    note: 'Krátká forma'
definice: 'Funkční celek hodinového stroje, jehož úkolem je **hrát melodii** (Westminster chime, vlastní motiv, lidovou píseň) sérií úderů [kladívek](/slovnik/kladivko) na různě naladěné [cymbály](/slovnik/cymbal) nebo zvony zvonkohry. Vlastní [lanový buben](/slovnik/lanovy-buben) a závaží, vlastní hřídel s vačkami nebo válcem s kolíčky, který určuje sekvenci úderů. U věžních hodin oddělené od [jicího stroje](/slovnik/jici-stroj) a [bicího stroje](/slovnik/bici-stroj) soukolí.'
pribuzne:
  - soukoli
  - jici-stroj
  - bici-stroj
  - cymbal
  - kladivko
  - westminster-chime
references:
  - key: spatny-1882
    title: 'Německo-český slovník pro hodináře a pouzdráře hodinářské'
    citace: 'Spielwerk — hrací stroj, zvonkohra.'
    zoteroKey: W6VEJ854
  - key: skala-2013
    title: 'Malé závěrkové kolečko na bicím stroji pražského orloje'
    citace: 'Vypouštění na palici změněno na vypouštění s náběhem, prováděné kolíčkem na hlavním kole zvonicího stroje.'
    pages: '228'
    zoteroKey: EQVW9VVP
---

## Výklad

**Soukolí zvonicí** (synonyma *zvonicí soukolí*, *zvonicí stroj*; též *soukolí zvonkohry*) je funkční celek hodinového stroje, který **hraje melodii** úderem [kladívek](/slovnik/kladivko) na různě naladěné [cymbály](/slovnik/cymbal) nebo zvony zvonkohry. Typický případ:

- **Westminster chime** (4 cymbály) odbíjející čtvrtě + celé hodiny — viz [Westminster chime](/slovnik/westminster-chime).
- **Vlastní melodie** (chorály, lidové písně) u větších věžních zvonkoher.
- **Petite & grande sonnerie** u luxusních stojacích a kapesních hodinek.

### Princip činnosti

1. **Vypouštění** — analogicky k [bicímu stroji](/slovnik/bici-stroj): kolíček na hlavním kole jicího stroje (nebo bicího) zvedne výpustnou páku zvonicího → soukolí se rozjede.
2. **Sekvence úderů** — hnaný hřídel nese **válec s kolíčky** (cylinder pin-barrel) nebo **kotouč s vačkami**; jak se otáčí, jednotlivé kolíčky postupně zvedají [kladívka](/slovnik/kladivko), která dopadají na příslušné [cymbály](/slovnik/cymbal) — vznikne melodie.
3. **[Větrník](/slovnik/vetrnik)** reguluje rychlost přehrávání.
4. **Zastavení** — po dokončení sekvence zapadne zapadací raménko a soukolí se zastaví.

### Vztah k jicímu a bicímu soukolí

U **velkých věžních hodin** (Pražský orloj, katedrály se zvonkohrou) je zvonicí soukolí **třetí nezávislý celek** vedle jicího a bicího. Skála 2013 popisuje souvislost vypouštění Pražského orloje:

> *„Vypouštění na palici změněno na vypouštění s náběhem, prováděné kolíčkem na hlavním kole **zvonicího stroje**."*

Zde tedy „zvonicí stroj" odbíjel doprovod ke Slunci/Měsíci v starší konfiguraci; dnešní orloj má zvonicí stroj nahrazen apoštolským strojem (figurkový pohyb).

U **stojanových hodin se zvonkohrou** (např. tatovské „Hořkohorské hodiny" s Westminsterem) je zvonicí soukolí v jednom rámu s jicím a bicím, ale fyzicky samostatné s vlastním pérem nebo závažím.

## Příbuzné termíny

[soukolí](/slovnik/soukoli) (obecně) · [jicí stroj](/slovnik/jici-stroj) · [bicí stroj](/slovnik/bici-stroj) · [cymbál](/slovnik/cymbal) · [kladívko](/slovnik/kladivko) · [Westminster chime](/slovnik/westminster-chime)

## Reference

> „**Spielwerk** — hrací stroj, zvonkohra."
> — ŠPATNÝ, František. *Německo-český slovník pro hodináře a pouzdráře hodinářské.* 1882.

> „Vypouštění na palici změněno na vypouštění s náběhem, prováděné **kolíčkem na hlavním kole zvonicího stroje**."
> — SKÁLA, Petr. *Malé závěrkové kolečko na bicím stroji pražského orloje.* Pokroky matematiky, fyziky a astronomie 58 (2013), č. 3, s. 228.
