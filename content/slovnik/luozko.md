---
title: lůžko (luožko měsícovo)
slug: luozko
kategorie: mechanika
varianty:
  - term: lůžko
    status: preferred
    note: 'Moderní cs strojírenský / hodinářský termín pro ložisko (pro čep hřídele). Specificky orlojový význam — držák kotouče měsíce — je staročes. užití (Táborský 1570).'
  - term: luožko
    status: archaic
    note: '16. století pravopis (Táborský 1570: „luožko měsícovo").'
    doloženo: 'Táborský 1570'
  - term: ložisko
    status: preferred
    note: 'Moderní cs synonymum k „lůžko"; obvyklé ve strojírenství.'
prekladyDe:
  - term: Lager
    genus: n
    zdroj: 'DE — ložisko (Špatný 1882, Berner FHS)'
  - term: Mondscheibenlager
    genus: n
    zdroj: 'DE specifický — lůžko měsíčního kotouče (orlojí)'
prekladyEn:
  - term: bearing
    zdroj: 'EN — ložisko'
  - term: moon disc bearing
    zdroj: 'EN — specifický orlojní význam'
prekladyFr:
  - term: palier
    genus: m
    zdroj: 'FR — ložisko'
  - term: support
    genus: m
    zdroj: 'FR — držák'
  - term: coussinet
    genus: m
prekladyLa:
  - term: lectulus
    genus: m
    zdroj: 'Balbín/Šlajer 1681'
definice: 'Drobné lůžko / držák kotouče měsíce. V něm rotuje kulička měsíce, čímž se na pukle (terči uprostřed číselníku) ukazuje fáze měsíce. V hodinkové terminologii odpovídá moderní pojem **ložisko** (en. bearing) — drobné kruhové pouzdro, ve kterém se otáčí čep hřídele; v náramkových hodinkách často **safírový kámen** (jewel bearing).'
pribuzne:
  - pukla
  - pravidlo-orloje
  - hridel
references:
  - bibKey: taborskyzklokotskehoryZpravaStaromestskemOrloji1570
    citace: "Luožko měsícovo a pod tím lůžkem na samém konci pravidlka udělán jest kroužek."
    note: "Rukopis AHMP, fond 3 (Sbírka úředních knih a rukopisů), sign. 1867."
  - bibKey: teigeJanaTaborskehoKlokotske1901
    note: "Tištěná edice s úvodem (J. Teige, Praha 1901)."
  - bibKey: spatny1882nemeckoCeskySlovnik
    title: 'Špatný F., *Německo-český slovník pro hodináře a pouzdráře hodinářské*. 1882.'
    note: 'Heslo Lager — DE odpovídající moderní cs „lůžko" / „ložisko". Specifický orlojní význam (lůžko měsíčního kotouče) viz Táborský 1570 a edice Teige 1901.'
    key: spatny-1882
---

## Výklad

Mechanismus pohyblivého kotouče měsíce — typický pro středověké astronomické orloje — je doložen u pražského, lyonského a štrasburského orloje. V moderní češtině: **měsíční kotouč** nebo **měsíční lůžko**.

## Příbuzné termíny

[pukla](/slovnik/pukla), [pravidlo-orloje](/slovnik/pravidlo-orloje)

## Obrázky

*(zatím chybí — k doplnění z fotodokumentace Staroměstského orloje nebo z edice Teige 1901)*
