---
title: posůvka
slug: posuvka
kategorie: bici
prekladyDe:
  - term: Schaltfinger
  - term: Schaltklinke
prekladyEn:
  - term: gathering pallet
  - term: lifting piece
prekladyFr:
  - term: palette d'élévation
definice: "Páková nebo prstová součást bicího stroje typu II (s početníkem), která při každém oběhu posune **početník** o jeden zub a tím umožní pravidelné odpočítávání úderů."
pribuzne:
  - bicí-stroj
  - početník
  - srdcovka
  - stupnice
---

## Výklad

Sladkovský 1947 zavádí termín v kontextu novějšího typu bicího stroje:

> „Místo kotoučku zapadacího čili srdcovky nese prodloužený čep tak zvanou posůvku, která posunuje jedním otočením početník vždy o jeden zub."

Posůvka tedy v moderním bicím stroji **funkčně nahradila srdcovku** ze starší konstrukce (typ I se závěrkovým kolem). Hlavní rozdíl: srdcovka pouze **zastavovala** stroj, posůvka navíc **inkrementuje** početník.

Tvar posůvky závisí na typu stroje:
- **Hodinová posůvka** — jednokřídlá, jeden zub
- **Půlová posůvka** (u hodin bijících i půle) — dvojitá, se dvěma křídly i zuby
- **Čtvrťová posůvka** — jednokřídlá, jeden zub (viz čtvrťový stroj)

Funkce posůvky **při bití**: kolo s posůvkou se otáčí synchronně s úderem kladívka. Křídlo posůvky během každého oběhu zvedne početník o jeden zub. Když poslední zub početníku padá za posůvku, **kolíček zastavovací** zaklesne, a stroj se zastaví.

## Příbuzné termíny

[bicí stroj](#bicí-stroj), [početník](#početník), [srdcovka](#srdcovka), [stupnice](#stupnice), [kolíček zastavovací]

## Reference

> „Místo kotoučku zapadacího čili srdcovky nese prodloužený čep tak zvanou posůvku, která posunuje jedním otočením početník vždy o jeden zub."
> — SLADKOVSKÝ, Josef. *Učebnice odborné nauky hodinářské.* 1947, Bicí stroj s početníkem a stupnicí. [Zotero `VIBRCUZT`]

> „U hodin čtvrťových, opakovacích, jest hodinový stroj upraven jako u půlových bicích. Rozdíl pozůstává v tom, že početník má všechny zuby stejné výšky, západka stojí zde ve směru obráceném a má na opačné straně jednoramenné zakončení, posůvka je dvojitá se dvěma křídly i zuby."
> — SLADKOVSKÝ, Josef. *Učebnice odborné nauky hodinářské.* 1947. [Zotero `VIBRCUZT`]

> „Početník čtvrťový má jen 6–7 zubů, posůvka jedno křídlo a zub."
> — SLADKOVSKÝ, Josef. *Učebnice odborné nauky hodinářské.* 1947, popis čtvrťového stroje. [Zotero `VIBRCUZT`]

## Obrázky

*(zatím chybí — návrh: srovnávací schéma srdcovka × posůvka, popis čtvrťové posůvky)*
