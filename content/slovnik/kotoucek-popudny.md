---
title: kotouček popudný
slug: kotoucek-popudny
conceptId: HORO-ESCAPEMENT-010
kategorie: mechanika
varianty:
  - term: kotouček popudný
    status: preferred
    doloženo: 'Sladkovský 1947'
  - term: kotouček vidličky
    status: preferred
    note: 'Alternativní moderní termín zdůrazňující funkci (kotouček spolupracující s vidličkou kotvy).'
  - term: kotouč vahadla
    status: archaic
    note: 'Sušický 1900 — „vahadlo" jako starší výraz pro setrvačku.'
    doloženo: 'Sušický 1900'
prekladyDe:
  - term: Hebelscheibe
    genus: f
    zdroj: 'Berner, *Illustriertes Fachlexikon der Uhrmacherei* (heslo „Hebelscheibe")'
prekladyEn:
  - term: impulse roller
    zdroj: 'Berner, *Illustrated Professional Dictionary of Horology* (heslo „plate II, roller-table")'
  - term: roller
    zdroj: 'Berner — užíváno samostatně v kombinacích „single roller", „double roller"'
prekladyFr:
  - term: plateau
    genus: m
    zdroj: 'Berner, *Dictionnaire professionnel illustré de l''horlogerie* (heslo „plateau" II — organe de l''échappement)'
  - term: plateau d'impulsion
    genus: m
    zdroj: "Sladkovský 1947; podrobnější FR varianta"
definice: "Kotouč na hřídeli [setrvačky](/slovnik/setrvacka) s elipsoidním kamenem [vodítka](/slovnik/vodicko); spolu s vidličkou kotvy hlavní součást [kotvového kroku](/slovnik/krok) kapesních a náramkových hodinek."
pribuzne:
  - vodicko
  - setrvacka
  - kotva
  - vidlice
  - krok
references:
  - key: berner
    title: "Dictionnaire professionnel illustré de l'horlogerie / Illustrated Professional Dictionary of Horology"
    citace: "plateau simple: rondelle d ajustée sur l'axe du balancier et portant la cheville de plateau (ellipse) a. Dans l'échappement, le plateau reçoit, par sa cheville, les impulsions de l'ancre au balancier."
    note: 'dictionary.fhs.swiss (heslo „plateau" ID 2623).'
---

## Výklad

**Kotouček popudný** (alt. *kotouček vidličky*; Berner *plateau* / *Hebelscheibe* / *impulse roller*) je malý kotouč pevně nasazený na hřídeli [setrvačky](/slovnik/setrvacka). Nese **kámen [vodítka](/slovnik/vodicko)** (elipsu), který je hlavním zprostředkovatelem výměny energie mezi kotvou a setrvačkou.

### Jednoduchý vs. dvojitý kotouček (Berner)

Berner rozlišuje dvě varianty u švýcarského páčkového kroku:

- **Jednoduchý kotouček** (plateau simple / single roller) — jediný kotouček *d* nese vodítko *a*; přijímá popud z [vidlice](/slovnik/vidlice) kotvy a brání jejímu nezávislému pohybu, zatímco setrvačka opisuje doplňkový oblouk.
- **Dvojitý kotouček** (plateau-double / double roller) — kromě hlavního kotoučku *b* s vodítkem *a* je tu i **menší kotouček *c* s vrubem** (encoche), který slouží jako vodicí dráha pro **kopí (dart / guard-pin)** vidlice. Toto provedení mají **vyšší řady přesných hodinek**, protože dvojitý kotouček lépe brání nechtěnému přesmyku vidličky.

### Souvislost s [vidlicí kotvy](/slovnik/vidlice)

Kotouček popudný + vodítko + vidlička kotvy tvoří **trojici klíčových součástí kotvového kroku** přesných náramkových a kapesních hodinek. Jejich vzájemná geometrie (úhel záběru vidlice, hloubka vrubu, tvar kopí) určuje **bezpečnost a přesnost** kroku.

### Archaismus „kotouč vahadla"

Sušický 1900 používá ještě **„kotouč vahadla"** — „vahadlo" bylo starší české jméno pro setrvačku (z DE *Unruhe*; Špatný 1882 překládá „Unruhe f. = vahadlo, balancier, setrvačník"). Po roce 1947 (Sladkovský) ustupuje termínu „setrvačka" / „kotouček".

## Příbuzné termíny

[vodítko](/slovnik/vodicko) · [setrvačka](/slovnik/setrvacka) · [kotva](/slovnik/kotva) · [vidlice](/slovnik/vidlice) · [krok](/slovnik/krok)

## Reference

> „**plateau simple**: rondelle d ajustée sur l'axe du balancier et portant la cheville de plateau (ellipse) a. Dans l'échappement, le plateau reçoit, par sa cheville, les impulsions de l'ancre au balancier; d'autre part, il limite les déplacements de la fourchette pendant que le balancier décrit l'angle supplémentaire."
> — BERNER, G.-A. *Dictionnaire professionnel illustré de l'horlogerie.* Federation of the Swiss Watch Industry (FH). Online: dictionary.fhs.swiss (heslo „plateau" ID 2623).

> „**plateau-double**: pour assurer de meilleures fonctions, les échappements soignés ont, en plus du plateau b qui porte la cheville (ellipse) a, un plateau plus petit c avec encoche qui limite l'ébat du dard."
> — Tamtéž.
