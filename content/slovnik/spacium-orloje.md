---
title: spacium denní × noční
slug: spacium-orloje
kategorie: astronomicke
varianty:
  - spacium denní
  - spacium noční
definice: "Denní × noční pásmo na astronomickém ciferníku — barevně odlišená oblast (typicky lazurová pro den, tmavá modř nebo černá pro noc) ukazující, kdy slunce stojí nad obzorem (\"supra orizontem\") a kdy pod obzorem."
pribuzne:
  - sphera
  - supra-orizontem
isStub: true
references:
  - bibKey: taborskyzklokotskehoryZpravaStaromestskemOrloji1570
    citace: "Spacium denní na sphéře naštrajchovati lazourem světlejším a spacium noční temnějším."
    note: "Rukopis AHMP, fond 3 (Sbírka úředních knih a rukopisů), sign. 1867."
  - bibKey: teigeJanaTaborskehoKlokotske1901
    note: "Tištěná edice s úvodem (J. Teige, Praha 1901)."
---

## Výklad

Lat. *spatium* = prostor, pásmo. Na pražském orloji je toto členění dodnes viditelné: středová oblast (den) je jasně lazurová, vnější obvod (noc) je temně modrý. Mezilehlé pásmo *crepusculum* (= soumrak/úsvit) je zlatavé.

## Příbuzné termíny

[sphera](/slovnik/sphera), [supra-orizontem](/slovnik/supra-orizontem)

## Obrázky

*(zatím chybí — k doplnění z fotodokumentace Staroměstského orloje nebo z edice Teige 1901)*
