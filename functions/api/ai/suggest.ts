 ...
 *
 * Model: @cf/mistralai/mistral-small-3.1-24b-instruct (cs OK, ~$0.11 /M
 *        input). 10K neurons/day free → ~280 calls/day pro typical
 *        500-token kontext + 80-token suggestion.
 *
...
async function generateSuggestion(env: Env, text: string, limit: number): Promise<string> {
...
${text}`;
  ...
  try {
    const out = await env.AI.run(MODEL, {
      ...
      max_tokens: limit,
      temperature: 0.4, // ne-příliš-kreativní; držet konzervativně
    });
    // Cloudflare Workers AI vrací buď { response: '...' } (LLM) nebo
    // { result: { response: '...' } } (některé models). Normalizuj.
    const suggestion = (out.response ?? out.result?.response ?? '').trim();
    if (!suggestion) {
      throw new AIFallbackError('unknown', 'AI vrátila prázdnou response');
    }
    return suggestion;
  } catch (err) {
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  try {
    const suggestion = await generateSuggestion(env, text, limit);
